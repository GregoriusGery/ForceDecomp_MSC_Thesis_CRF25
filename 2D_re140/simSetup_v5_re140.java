...
public class simSetup_v5_re140 extends StarMacro {
    ...
    public void execute() {
        ...
        sim.println("Macro setup execution complete. The simulation is ready to run straight to 7.0s.");
    }

    private void setupPhysics() {
        ImportManager importManager = sim.getImportManager();
		// change to the mesh location directory
        importManager.importMeshFiles(new StringVector(new String[] {resolvePath("D:\\file\\file\\file\\mesh.ccm")}), NeoProperty.fromString("{\'FileOptions\': [{\'Sequence\': 42}]}"));
    ...
    private void createParameters() {
        ...
        dt.getQuantity().setValueAndUnits(2.5E-3, units_s);
        ...
        max_time.getQuantity().setValueAndUnits(7.0, units_s);
    ...
    private void createMonitors() {
        ...
        for (String var : meanVars) {
            try {
				...
				int startTimeStep = (int) Math.round((maxTimeVal - 2.0) / timeStepVal);
